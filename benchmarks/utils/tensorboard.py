...
def main(folder: str, out_dir: str):
    ...
    for model in os.listdir(folder):
        for version in os.listdir(os.path.join(folder, model)):
            logdir_or_logfile = os.path.join(folder, model, version)
            ...
            all_logs['model'] = model
            all_logs['version'] = version
            print(model, version)
            dfs.append(all_logs)

    os.makedirs(out_dir, exist_ok=True)
    print("saving to csv file")
    out_file = os.path.join(out_dir, "logs.csv")
    print(out_file)
    pd.concat(dfs).to_csv(out_file, index=None)
